...
(function($){
	$.fn.crossword = function(entryData) {
			...
			this.after('</div><div class="col s12 m12 l6" ><div id="puzzle-clues"><h2>Across</h2><ul id="across"></ul><h2>Down</h2><ul id="down"></ul></div></div><div class="row center offset-s3 s4"><input type="submit" class="sbt" onclick="return submit();" value="Check Answers" ></input></div>');
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							if(x > 1 ){
								...
								if($(light).empty())
								{
		$(light).addClass('entry-' +puzz.data[x-1].position).append('<input maxlength="1" class="browser-default" val="" type="text" tabindex="-1" />');
								};
